...
const getTotalRewardByAddress = async (): Promise<void> => {
  ...
  if (result.data && result.data.success) {
        ...
        result.data.transactions.forEach((tx) => {
          if (!operatorAccountMap[tx.wrappedEVMAccount.txFrom]) {
            operatorAccountMap[tx.wrappedEVMAccount.txFrom] = Web3.utils.toBN(
              Number('0x' + tx.wrappedEVMAccount.readableReceipt.stakeInfo.reward).toString()
            )
            ...
            operatorAccountMap[tx.wrappedEVMAccount.txFrom] = operatorAccountMap[tx.wrappedEVMAccount.txFrom].add(
              Web3.utils.toBN(Number('0x' + tx.wrappedEVMAccount.readableReceipt.stakeInfo.reward).toString())
            )
          }
          // console.log(
    ...
    for (const operatorAcc of Object.keys(sortedOperatorAccountMap)) {
      ...
      sortedOperatorAccountMap[operatorAcc] = Web3.utils.fromWei(BNvalue, 'ether')
    }

    // console.log(sortedOperatorAccountMap)

    fs.writeFileSync('operatorAddressTotalRewardMap.json', JSON.stringify(sortedOperatorAccountMap, null, 2))
  }
