...
const STAKE_TX_TYPE = TransactionSearchType.StakeReceipt
  ...
  if (result.data && result.data.success) {
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      if (result.data && result.data.success) {
        for (const tx of result.data.transactions) {
          const nodeAddress = tx.wrappedEVMAccount.readableReceipt.stakeInfo.nominee
    ...
    for (let operatorAddress of Object.keys(operatorAddresses)) {
      ...
      for (const nodeAddress of operatorAddresses[operatorAddress]) {
        ...
        if (result.data && result.data.success && result.data.accounts && result.data.accounts.length > 0) {
      ...
      if (count % limit === 0) {
        fs.writeFileSync(`nodeAccountByoperatorAddress${page}.json`, JSON.stringify(nodeAccounts, null, 2))
        page++
        await sleep(2000)
        nodeAccounts = {}
      }
